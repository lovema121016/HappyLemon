...
{
    public partial class client_suoyin1 : Form
    {
        public DataGridView data = new DataGridView();
        public ShoukuanDan shoukuan;

        public string type;
        public client_suoyin1()
        {
            InitializeComponent();
        }

        private void client_suoyin1_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < data.Rows.Count; i++)
            {
                ...
                try
                {
                    if (Convert.ToBoolean(this.data.Rows[i].Cells[0].Value) == true)
                    {
                        if (this.type == "收款单")
                        {
                           shoukuan.textBox1.Text = data.Rows[i].Cells[1].Value.ToString() + " " + data.Rows[i].Cells[2].Value.ToString();
                            this.Close();
                        }
